...
export default function Infopage() {
  ...
  const cssofprod2 = "sm:w-60 w-40 text-[12px] sm:text-[15px] font-bold text-wrap";
  
  const iconcss = "text-cyan-700 w-5 h-5 sm:w-[30px] sm:h-[30px]";
  const btncss = "w-full h-12 md:w-full md:h-8 md:mb-1 lg:h-12 rounded-md cursor-pointer font-semibold border-black border-2 mb-5";
...
async function fetchProductAndRelated() {
  ...
  return (
    <div className="mb-10">
      ...
      <div className="flex flex-col sm:flex-row md:my-1 sm:my-10">
      ...
      <div className="flex flex-col px-4 sm:px-8 md:w-[55%] lg:w-[60%] xl:w-[65%]">
          ...
          <h1 className="text-[20px] underline sm:no-underline lg:text-3xl pt-4">
            {productInfo.title}
          </h1>
          <h2 className="text-1xl md:text-sm lg:text-2xl lg:mr-5 pt-2 sm:pt-4 text-slate-500">
            {productInfo.description}
          </h2>
          <h3 className="lg:text-xl text-1xl uppercase pt-2 sm:pt-4 text-zinc-800">
          ...
          <div className="text-2xl pt-1 sm:pt-2 text-black">
            <div className="flex items-center gap-4">
              ...
              <span className="text-black text-sm lg:text-lg">${Math.floor(price)}</span>
            </div>
          ...
          <div className="flex flex-wrap mt-3 justify-between sm:justify-center sm:gap-6">
            {[
              ...
              <div
                key={i}
                className="w-1/2 flex items-center gap-2 lg:mb-4 mb-1 sm:w-auto sm:flex-col sm:items-center sm:text-center"
              >
                <div className="sm:w-14 sm:h-14 md:w-6 md:h-6 lg:w-14 lg:h-14 sm:rounded-full sm:bg-gray-100 sm:shadow-md sm:flex sm:items-center sm:justify-center">
                  {item.icon}
                </div>
                <p className="text-sm font-medium md:text-[12px]">{item.text}</p>
              </div>
            ))}
          </div>
          ...
          <div className="mt-2">
            <label className="text-2xl md:text-xl text-black">Quantity</label>
          ...
          <hr className="mt-3" />
          <div className="grid grid-cols-2 text-wrap mt-2 gap-1 mb-4 capitalize">
              <span className={cssofprod2}>Brand</span>
              <span className={cssofprod}>{productInfo.brand}</span>
              <span className={cssofprod2}>Warranty </span>
